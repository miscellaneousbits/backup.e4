...
{
    return (le32_to_cpu(bm[index / BM_ENTRY_BITS]) >> (index % BM_ENTRY_BITS)) &
           1;
}

static inline void set_bm(bm_entry_t* bm, u64 index, u32 value)
{
    bm[index / BM_ENTRY_BITS] |=
        le32_to_cpu((value & 1) << (index % BM_ENTRY_BITS));
}

FILE* part_open(char* name, u32 write);
...
gzFile dump_open(char* dump_fn, u32 compression, u32 write);
